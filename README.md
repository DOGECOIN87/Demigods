...
**Phase 3 uses a locked 1254 × 1254 canvas. The collection renders: combination space 1682104320, no active blocker. Hair recolours DG-169–175, face recolours DG-177–197 and the front-hair fringe DG-229–236 are interim placeholders pending painted designs.**

Outfits are six painted robes (DG-199–204), which replaced the interim procedural coats on 2026-07-27 — but they measure 0.82–0.92 silhouette IoU against each other, so they are one garment in six colourways. Ten silhouette-driven replacement briefs are in `prompts/24_outfit_silhouettes.md` (DG-250–259). See `docs/qa/painted_outfits_2026-07-27.md` — including the measurement showing that any sleeved outfit hides 34–83% of what distinguishes the five base poses.

The base bodies are faceless as of 2026-07-27. The `eyes`, `eyebrows` and `mouths` categories supply those features, recovered from the pre-removal master as isolated layers. See `docs/qa/face_layer_conflict_2026-07-27.md` and `docs/qa/face_recovery_2026-07-27.md`.

`hair_front` was populated on 2026-07-27 after measurement showed `hair_back` covers 0% of the scalp, so every token had been rendering bald. That also brought the collection's first compatibility rules, pairing each fringe with its matching back-hair colour. See `docs/qa/hair_front_2026-07-27.md`.

`mouths` went from one asset to thirteen on the same day (DG-237–248, `docs/qa/mouths_2026-07-27.md`), and backgrounds from four to twelve (DG-011–014 and DG-158–160 plus DG-249; `docs/qa/backgrounds_005_008_2026-07-27.md` and `docs/qa/backgrounds_round_two_2026-07-27.md`).

Only three categories change what a 210 px thumbnail looks like — backgrounds, outfits and hair colour. Measure it with `python scripts/build_token_sheet.py --salience` before trusting the combination-space number. Every registered asset is catalogued in `docs/qa/composites/asset_catalogue.png` via `python scripts/build_asset_catalogue.py`.
...
The registry currently contains **88 registered assets**: 12 backgrounds, 6 rear_auras, 8 hair_back, 5 base_bodies, 6 outfits, 13 eyes, 9 eyebrows, 13 mouths, 8 expression_marks, 8 hair_front.

Backlog: **253** distinct assets across 16 categories; **2** complete (base_bodies, expression_marks).
...
The registry contents are summarised at the top of this file. The checker requires every registered file to exist at the declared production path, remain `production_ready`, match its declared SHA-256 and dimensions, and pass category-aware PNG QA.
...
The reporter derives per-category registered, remaining, and completion counts from `assets/asset_manifest.json` and `docs/trait-production-backlog.md`, rewrites the generated blocks in `docs/production_status.md` and the registry summary above, and fails when a backlog row marked `registered` is missing from the manifest, when the manifest registers a path no backlog row claims, or when `pending_categories` names a category whose assets are all registered.
