...
Production is presently blocked at the approved neutral master. Follow `docs/production-status.md` and Issue #4 before creating character-aligned assets. Never approximate the locked avatar from the damaged WebP or reference-sheet previews.
...
3. Update `docs/production-status.md`.
...
During preproduction, when canonical production folders are still empty:
...
The validator checks complete binary decoding, PNG format, dimensions, alpha behavior, visible bounds, filename/category agreement, sequential numbering, and manifest consistency.
...
- manifest and production-ledger entries
