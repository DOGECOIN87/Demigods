...
**Phase 3 — Renderable and clothed; painted trait production continues**

Combination space is **1682104320** against the 777 supply. A dry run produced 777 unique verified tokens and every one reads as clothed at thumbnail size.

**No active blocker.** The outfit blocker was cleared on 2026-07-27 by five interim procedural coats, which is why `outfits` sits in `generate_777.REQUIRED_CATEGORIES` — the requirement stays so the category can never empty again.

Those coats have since been **replaced and removed**. `outfits` now holds six painted robes supplied by the collection owner (DG-199–204), each a distinct design rather than a recolour. The procedural coats were flat and near-identical; their files, registrations and backlog rows are gone, though `scripts/build_outfit.py` is kept. The `OUTFIT` sheet cells DG-037–046 are still pending, since these robes are not those designs. See `docs/qa/painted_outfits_2026-07-27.md`.

The six are also **one silhouette**: measured against each other they run 0.823–0.920 IoU, median 0.894, with width profiles agreeing row by row. Palette varies, shape does not — the same failure the mouths had. `intake_painted_outfit.py` now reports silhouette IoU against every registered outfit and rejects anything at or above 0.75. Ten briefs specifying silhouette rather than colour are queued as DG-250–259 in `prompts/24_outfit_silhouettes.md`.

That QA report also records a finding that was not previously measured: an outfit composites over the base body, so its sleeves hide 34–83% of what distinguishes each base pose from the neutral master — under the old procedural coats as well as the new robes. The five poses are far closer to interchangeable than their count implies.

Seven interim hair recolours (DG-169–175) followed on the same basis, after a random sample showed identical hair on every token. They vary colour only — the silhouette is DG-031's in all seven — so DG-029–036 also remain pending. See `docs/qa/hair_recolours_2026-07-27.md`.

The face followed the same pattern on the same day. The baked eyes, eyebrows and mouth were erased from all five base bodies so the three face categories could composite over them under the ordinary isolation rule, then recovered from the pre-removal master as isolated layers and recoloured into twelve eye and eight eyebrow palettes (DG-176–198). Palettes are gated on measured contrast against the skin and against every registered background. Again colour varies and shape does not, so the 52 painted `FACE` sheet cells — DG-055–106 — remain pending. See `docs/qa/face_layer_conflict_2026-07-27.md` and `docs/qa/face_recovery_2026-07-27.md`.

`hair_front` was empty until 2026-07-27, and measurement showed that mattered more than an empty category usually does: `hair_back` covers **0%** of the scalp at every row from Y 150 to Y 340, so every token rendered as a bald head. Eight procedural fringe layers (DG-229–236) clear it. They also brought the collection's first compatibility rules — `config/compatibility.json` had held zero since it was written — pairing each fringe with its matching back-hair colour after a random sheet put a gold fringe on red hair. Again colour varies and shape does not, so the eight painted `HAIR` cuts DG-115–122 remain pending. See `docs/qa/hair_front_2026-07-27.md`.

`mouths` held a single asset until 2026-07-27 — every token would have worn the same expression. Twelve procedural mouths (DG-237–248) take it to thirteen, drawn from distance fields on the locked anchor with ink sampled from the recovered mouth so both read as the same hand. The twelve painted `FACE` mouth cells DG-095–106 remain pending. See `docs/qa/mouths_2026-07-27.md`.

Backgrounds 005–008 were registered on 2026-07-27 (DG-011–014), taking the category from four to eight. They are the highest-value assets in the collection per unit of work: `scripts/build_token_sheet.py --salience` shows backgrounds move a 210 px thumbnail by 47.1 against 11.8 for outfits, 4.9 for hair and under 1 for every face category. A second wave followed the same day — world tree, infinite arcane library, ember ruins and a bioluminescent grove (DG-158–160, DG-249) — taking the category to twelve and distinct thumbnail appearances from 192 to 576. Skybound isles was held: its frame bottom is falling water, so the feet and floor ring hang in open air, confirming the backlog's earlier floor-plane finding. See `docs/qa/backgrounds_round_two_2026-07-27.md`. No native 1254 render existed, so the 1024 references were upscaled under an owner waiver; the measured cost of that route after the category's 3.0 px blur is 0.24/255. See `docs/qa/backgrounds_005_008_2026-07-27.md`.
...
Backlog status tally: pending 165, candidate 0, QA-failed 0, approved 0, registered 89.
...
- Procedural mouth builder; category no longer one asset wide
