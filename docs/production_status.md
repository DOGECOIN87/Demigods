...
**Ready to generate, 2026-09-25.** Every category the generator draws from is populated and
passes preflight; the rule-valid space is about 1.56 trillion combinations, so the 770
generative tokens (or the 700 the owner needs) use 0.0% of it. That day:

- Twenty **dressed-body outfits** were registered: four families (black hooded robe, brown
  leather coat, olive ragged cloak, navy high-collar coat) in all five poses, painted by the
  owner with the body intact. Each binds its base pose and hides it in the render, so the
  fit defects of garment-over-base layers cannot occur. Outfits 006-009's single-pose layers
  are retired in their favour. Outfits 001-005 and 010 remain as single-pose layers until
  dressed renders replace them; prompts for the thirty missing renders are in
  `prompts/dressed_body_pose_pack_2026-09-25.md`, intake in `docs/workflows/dressed_body_intake.md`,
  QA in `docs/qa/dressed_bodies_2026-09-25.md`.
- **Neckwear was omitted** by the owner. Its eight rows close as `withdrawn`, a status for rows
  closed by recorded decision rather than registration.
...
- Registered production assets: **119**
- Distinct backlog assets: **120** (plus 60 withdrawn by recorded decision)
- Completed categories: **10 of 11** (backgrounds, hair_back, base_bodies, outfits, eyes, eyebrows, mouths, expression_marks, hair_front, global_finish)
- Withdrawn categories: rear_auras, neck_accessories, head_accessories, hand_objects, front_auras

| Category | Registered | Backlog assets | Remaining | State |
|---|---|---|---|---|
| `backgrounds` | 8 | 8 | 0 | complete |
| `rear_auras` | 0 | 0 | 0 | withdrawn |
| `back_accessories` | 7 | 8 | 1 | in progress |
| `hair_back` | 8 | 8 | 0 | complete |
| `base_bodies` | 5 | 5 | 0 | complete |
| `outfits` | 20 | 20 | 0 | complete |
| `neck_accessories` | 0 | 0 | 0 | withdrawn |
| `eyes` | 24 | 24 | 0 | complete |
| `eyebrows` | 16 | 16 | 0 | complete |
| `mouths` | 12 | 12 | 0 | complete |
| `expression_marks` | 8 | 8 | 0 | complete |
| `hair_front` | 8 | 8 | 0 | complete |
| `head_accessories` | 0 | 0 | 0 | withdrawn |
| `hand_objects` | 0 | 0 | 0 | withdrawn |
| `front_auras` | 0 | 0 | 0 | withdrawn |
| `global_finish` | 3 | 3 | 0 | complete |

Backlog status tally: pending 0, candidate 0, QA-failed 1, approved 0, registered 120, withdrawn 60.
...
## Supply saturation — resolved 2026-09-07

Registering DG-123 … DG-137 opened two whole categories and both are optional, so they
multiply the space rather than constraining it. `scripts/generate_777.py --preflight-only`
now reports a rule-valid space of **300,478,464** and saturation of **0.0%**, against 800 and
97% before. The section below records the original finding.

## Supply saturation — the former blocker (historical)
...
## Head accessories and hand objects — registered 2026-09-07

Both categories completed in one pass. The candidates already existed and had passed binary
intake, the trait rig gate and a recorded visual review; what they had not passed was a check
that they were seated on the anatomy they belong to, which no automated gate performs.

Ten head accessories were normalized at one fixed placement — 520 px wide, top at Y 129 — so
every one was wider than the character's 329 px head and several floated above the skull or
crossed the eyes. Each is now re-derived from its immutable source in a single reduction at
the width and seat its own design needs, and every head-contact band clears the eyebrow line
at Y 308–323.

Five hand objects sat on the retired shared anchor X 404, which the 2026-08-15 recalibration
established lands at the wrist. All five now use the measured grip contacts — pose 002
`(438, 772)`, pose 004 `(438, 748)` — with the same 12° outward lean the registered pose-002
objects carry, and each is bound to its pose in `config/compatibility.json`.

Both categories are optional (`head_accessories` 0.55, `hand_objects` 0.60): a token wearing no
crown and holding nothing is a normal token.

Full record and before/after approval sheets: `docs/qa/aligned_candidates_2026-09-07/`.

## Undergarment coverage — corrected 2026-09-07

`scripts/hide_undergarment.py` carried a hardcoded list of five 1:1 base/outfit pairs. Outfits
006–010 were registered later, all bound to the neutral master, and none was in that list, so
four of them left the base's neutral tank showing. Pairs now come from
`config/compatibility.json` so a newly bound outfit cannot be missed.

Fit gaps — narrow strips where a garment fails to meet the arm and a tank strap shows — are
repainted; outfit_010 went to zero and both of outfit_009's shoulder slivers are gone. Neckline
openings the outfit means to leave open are enclosed by more tank with no skin to diffuse from,
so they are left alone and the tank reads there as a linen undershirt. Covering those means the
outfit carrying its own inner garment, which is a re-render rather than an edit.
