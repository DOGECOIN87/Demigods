...
# `withdrawn` closes a row by recorded decision rather than by registration: the
# asset is out of the collection (the owner dropped the category, or a newer
# asset superseded it) and is no longer remaining work. It must not be
# registered, and the decision belongs in the row and in the manifest's
# blocked_assets, not only in this vocabulary.
WITHDRAWN = "withdrawn"
BACKLOG_STATUSES = ("pending", "candidate", "QA-failed", "approved", "registered", WITHDRAWN)
...
def build_status(
    ...
    withdrawn_paths = {entry.path for entry in entries if entry.status == WITHDRAWN}
    ...
    for path in sorted(withdrawn_paths & registered_backlog_paths):
        errors.append(f"{path} is both withdrawn and registered in the backlog")
    ...
    for category in layer_order:
        ...
        active_entries = [entry for entry in category_entries if entry.status != WITHDRAWN]
        active_paths = {entry.path for entry in active_entries}
        registered_count = sum(
            1 for path, value in registered.items() if value == category and path in active_paths
        )
        backlog_total = len(active_paths)
        remaining = backlog_total - registered_count

        # A category with no backlog rows is source-gated: nothing is designed yet.
        # One whose every row is withdrawn is out of the collection by decision.
        ...
        if state not in ("complete", WITHDRAWN):
    ...
    return ProductionStatus(
        ...
        backlog_total=len({entry.path for entry in entries if entry.status != WITHDRAWN}),
        withdrawn_total=len(withdrawn_paths),
...
def render_block(status: ProductionStatus) -> str:
    ...
    withdrawn = [item.category for item in status.categories if item.state == WITHDRAWN]
    in_scope = len(status.categories) - len(withdrawn)
    lines = [
        BEGIN_MARKER,
        "",
        ...
        + (f" (plus {status.withdrawn_total} withdrawn by recorded decision)"
           if status.withdrawn_total else ""),
        f"- Completed categories: **{len(complete)} of {in_scope}**"
        + (f" ({', '.join(complete)})" if complete else ""),
    ]
    if withdrawn:
        lines.append(f"- Withdrawn categories: {', '.join(withdrawn)}")
    lines.extend([
        "",
...
def replace_block(document: str, block: str) -> str:
    ...
    if start == -1 or end == -1:
        raise ValueError(
            f"status document is missing the {BEGIN_MARKER} / {END_MARKER} generated region"
        )
    if end < start:
        raise ValueError(f"{END_MARKER} appears before {BEGIN_MARKER} in the status document")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.write or args.check:
        try:
            document = args.status_doc.read_text(encoding="utf-8")
            updated = replace_block(document, block)
        except (OSError, ValueError) as exc:
            status.errors.append(str(exc))
        else:
            if args.write and updated != document:
                args.status_doc.write_text(updated, encoding="utf-8")
                print(f"Updated generated ledger in {args.status_doc}")
            elif args.check and updated != document:
                document_stale = True
                status.errors.append(
                    f"{args.status_doc} generated ledger is stale; run `{REGENERATE_COMMAND}`"
                )
